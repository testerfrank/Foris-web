#!/usr/bin/env python
# _*_ coding:utf-8 _*_
import win32con
import win32gui
from selenium import webdriver
from selenium.webdriver import ActionChains
from selenium.webdriver.chrome.webdriver import WebDriver
from selenium.webdriver.support.select import Select
from selenium.webdriver.support.wait import WebDriverWait

from config.conf import cm
from utils.utils_logger import log
from utils.utils_time import sleep


class BasePage:
    """PageObject基类：对selenium常用方法的二次封装"""

    def __init__(self, driver: WebDriver = None):
        if driver is None:
            self._driver = webdriver.Chrome()
        else:
            self._driver = driver

    def imp_wait(self, seconds=3):
        """隐式等待"""
        log.info(f"隐式等待：{seconds}")
        self._driver.implicitly_wait(seconds)

    def wait_for_element(self, timeout, locator):
        """显式等待"""
        log.info(f"显示等待{locator}：{timeout}")
        WebDriverWait(self._driver, timeout).until(lambda x: x.find_element(*locator))

    def wait_for_condition(self, timeout, condition):
        """显式等待条件"""
        log.info(f"显示等待条件：{condition}")
        WebDriverWait(self._driver, timeout).until(condition)

    def condition(self, locator):
        """为了等待A元素，可以先判断点击A元素后返回的页面元素B是否可点击,locator为B元素；与wait_for_condition结合使用"""
        element_len = len(self.finds(locator))
        if element_len <= 0:
            self.find_and_click(locator)
        return element_len > 0

    def find(self, locator):
        """查找单个元素"""
        log.info(f"查找单个元素：{locator}")
        element = self._driver.find_element(*locator)
        self.high_light_element(element)
        return element

    def finds(self, locator):
        """查找元素"""
        log.info(f"查找元素：{locator}")
        elements = self._driver.find_elements(*locator)
        return elements

    def find_and_click(self, locator):
        """点击元素"""
        log.info(f"点击元素：{locator}")
        self.find(locator).click()

    def find_and_send(self, locator, text):
        """输入文本"""
        log.info(f"{locator}输入文本：{text}")
        self.find(locator).send_keys(text)

    def find_and_clear(self, locator):
        """清空文本"""
        log.info(f"清空文本：{locator}")
        self.find(locator).clear()

    def get_text(self, locator):
        """获取元素文本"""
        value = self.find(locator).text
        log.info(f"获取元素文本：{value}")
        return value

    def get_attribute(self, locator, name):
        """获取元素属性值"""
        value = self.find(locator).get_attribute(name)
        log.info(f"获取{locator}的{name}属性值为：{value}")
        return value

    def is_displayed(self, locator):
        """检查元素是否可见"""
        log.info(f"检查元素是否可见：{locator}")
        return self.find(locator).is_displayed()

    def is_enabled(self, locator):
        """检查元素是否可点击"""
        log.info(f"检查元素是否可点击：{locator}")
        return self.find(locator).is_enabled()

    def is_selected(self, locator):
        """检查元素是否已被选择"""
        log.info(f"检查元素是否已被选择：{locator}")
        return self.find(locator).is_selected()

    def move_to_element(self, locator):
        """鼠标悬停在元素上"""
        log.info(f"鼠标悬停在元素上：{locator}")
        ActionChains(self._driver).move_to_element(self.find(locator)).perform()

    def drag_and_drop(self, locator1, locator2):
        """拖动元素"""
        log.info(f"拖动元素：source:{locator1},target:{locator2}")
        ActionChains(self._driver).drag_and_drop(self.find(locator1), self.find(locator2)).perform()

    def context_click(self, locator):
        """右键单击"""
        log.info(f"右键单击：{locator}")
        ActionChains(self._driver).context_click(self.find(locator)).perform()

    def double_click(self, locator):
        """鼠标双击"""
        log.info(f"鼠标双击：{locator}")
        ActionChains(self._driver).double_click(self.find(locator)).perform()

    def click_and_hold(self, locator):
        """鼠标长按"""
        log.info(f"鼠标长按：{locator}")
        ActionChains(self._driver).click_and_hold(self.find(locator)).perform()

    @property
    def title(self):
        """获取浏览器窗口标题"""
        value = self._driver.title
        log.info(f"获取浏览器窗口标题：{value}")
        return value

    @property
    def current_url(self):
        """获取浏览器当前窗口url"""
        value = self._driver.current_url
        log.info(f"获取浏览器当前窗口url：{value}")
        return value

    @property
    def page_source(self):
        """获取网页资源"""
        value = self._driver.page_source
        log.info(f"获取网页资源")
        return value

    def forward(self):
        """浏览器前进"""
        log.info("浏览器前进")
        self._driver.forward()

    def back(self):
        """浏览器后退"""
        log.info("浏览器后退")
        self._driver.back()

    def refresh(self):
        """浏览器刷新"""
        log.info("浏览器刷新")
        self._driver.refresh()

    def close(self):
        """关闭页签"""
        log.info("关闭页签")
        self._driver.close()

    def quit(self):
        """退出浏览器"""
        log.info("退出浏览器")
        self._driver.quit()

    @property
    def current_window_handle(self):
        """获取当前窗口句柄"""
        handle = self._driver.current_window_handle
        log.info(f"获取当前窗口句柄：{handle}")
        return handle

    @property
    def window_handles(self):
        """获取浏览器所有窗口句柄"""
        handles = self._driver.window_handles
        log.info(f"获取浏览器所有窗口句柄：{handles}")
        return handles

    def switch_to_window(self, handle):
        """切换窗口"""
        log.info(f"切换窗口：{handle}")
        self._driver.switch_to.window(handle)

    def switch_to_frame(self, frame_id):
        """切换frame"""
        log.info(f"切换frame：{frame_id}")
        self._driver.switch_to.frame(frame_id)

    def switch_to_parent_frame(self):
        """切换到上一层frame"""
        log.info("切换到上一层frame")
        self._driver.switch_to.parent_frame()

    def switch_to_default_content(self):
        """切换到主页面"""
        log.info("切换到主页面")
        self._driver.switch_to.default_content()

    def alert_text(self):
        """获取alert弹窗文本"""
        value = self._driver.switch_to.alert.text
        log.info(f"获取alert弹窗文本：{value}")
        return value

    def alert_send(self, text):
        """alert弹窗中输入文本"""
        log.info(f"alert弹窗中输入文本：{text}")
        self._driver.switch_to.alert.send_keys(text)

    def alert_accept(self):
        """确认alert弹窗"""
        log.info("确认alert弹窗")
        self._driver.switch_to.alert.accept()

    def alert_dismiss(self):
        """取消alert弹窗"""
        log.info("取消alert弹窗")
        self._driver.switch_to.alert.dismiss()

    def select_options(self, locator):
        """获取下拉框所有选项"""
        options = Select(self.find(locator)).options
        log.info(f"获取下拉框所有选项：{options}")
        return options

    def select_all_selected_options(self, locator):
        """获取下拉框所有已被选择的选项"""
        options = Select(self.find(locator)).all_selected_options
        log.info(f"获取下拉框所有已被选择的选项：{options}")
        return options

    def select_by_index(self, locator, index):
        """选中选项"""
        log.info(f"选中选项：{index}")
        Select(self.find(locator)).select_by_index(index)

    def select_by_value(self, locator, value):
        """选中选项"""
        log.info(f"选中选项：{value}")
        Select(self.find(locator)).select_by_value(value)

    def select_by_visible_text(self, locator, text):
        """选中选项"""
        log.info(f"选中选项：{text}")
        Select(self.find(locator)).select_by_visible_text(text)

    def select_deselect_by_index(self, locator, index):
        """取消选项"""
        log.info(f"取消选项：{index}")
        Select(self.find(locator)).deselect_by_index(index)

    def select_deselect_by_value(self, locator, value):
        """取消选项"""
        log.info(f"取消选项：{value}")
        Select(self.find(locator)).deselect_by_value(value)

    def select_deselect_by_visible_text(self, locator, text):
        """取消选项"""
        log.info(f"取消选项：{text}")
        Select(self.find(locator)).deselect_by_visible_text(text)

    def select_deselect_all(self, locator):
        """取消所有选项"""
        log.info("取消所有选项")
        Select(self.find(locator)).deselect_all()

    def upload_file(self, locator, file_path):
        """非input标签的文件上传"""
        log.info("文件上传")
        self.find_and_click(locator)
        sleep(2)
        # 一级顶层窗口
        dialog = win32gui.FindWindow("#32770", "打开")
        # 二级窗口
        comboBoxEx32 = win32gui.FindWindowEx(dialog, 0, "ComboBoxEx32", None)
        # 三级窗口
        comboBox = win32gui.FindWindowEx(comboBoxEx32, 0, "ComboBox", None)
        # 四级窗口 -- 文件路径输入区域
        edit = win32gui.FindWindowEx(comboBox, 0, "Edit", None)
        # 二级窗口 -- 打开按钮
        button = win32gui.FindWindowEx(dialog, 0, "Button", None)
        # 1、输入文件路径
        win32gui.SendMessage(edit, win32con.WM_SETTEXT, None, file_path)
        # 2、点击打开按钮
        win32gui.SendMessage(dialog, win32con.WM_COMMAND, 1, button)

    def screen_shot(self):
        """截图"""
        log.info("截图")
        self._driver.save_screenshot(cm.screen_shot_file)

    def execute_js(self, js, *args):
        """执行JavaScript脚本"""
        log.info(f"执行JavaScript脚本：{js}")
        self._driver.execute_script(js, *args)

    def scroll(self, x, y):
        """滚动页面"""
        log.info(f"滚动页面：{x},{y}")
        self.execute_js(f"window.scrollTo({x},{y})")

    def scroll_element(self, locator):
        """滚动查找元素"""
        log.info(f"滚动查找元素：{locator}")
        element = self.find(locator)
        return self.execute_js("arguments[0].scrollIntoView()", element)

    def display_element(self, locator):
        """显示被隐藏的元素"""
        log.info(f"显示被隐藏的元素：{locator}")
        element = self.find(locator)
        self.execute_js('arguments[0].style.display="block"', element)

    def high_light_element(self, element):
        """高亮显示元素"""
        log.info(f"高亮显示元素：{element}")
        self.execute_js("arguments[0].setAttribute('style',arguments[1])", element, "border:2px solid red")
